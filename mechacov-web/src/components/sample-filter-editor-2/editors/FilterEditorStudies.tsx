...
class Component extends React.PureComponent<ComponentProps, ComponentState> {
    ...
    private readonly fetchAllValues = async () => {
        ...
        const studies = MechACovClient.getStudies(this.abortController.signal).then((values: string[]) => {
...
const mapStateToProps = (state: any) => ({
    values: (state.sampleFilter.studyIds || []) as string[],
});


const mapDispatchToProps = (dispatch: any) => ({
    resetValues: () => { dispatch(SampleFilterActions.resetStudies()); },
    addValue: (value: string) => { dispatch(SampleFilterActions.addStudy(value)); },
    removeValue: (value: string) => { dispatch(SampleFilterActions.removeStudy(value)); },
